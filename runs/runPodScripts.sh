...
EXPERIMENT_NAME="d${DEPTH}-${NUM_ITERATIONS}iter-${GPU_TYPE}-B${TOTAL_BATCH_SIZE}-b${DEVICE_BATCH_SIZE}"
...
echo "Max wall time:    ${MAX_WALL_CLOCK_TIME} s"
...
python -m scripts.base_train_batch_size_schedule \
  --depth="${DEPTH}" \
  ...
  --eval-every=5000 \
  --resize-wall-clock-time=30

echo
echo "=== Finished ==="
echo "Log saved to: ${OUTPUT_FILE}"
echo "Training CSV: ${NANOCHAT_BASE_DIR}/base_checkpoints/d${DEPTH}/training_log.csv"
